...
const REPL_COMMANDS: ReplCommand[] = [
  ...
  { name: '/quit', description: '退�?Nova' },
  { name: '/clear', description: '清空对话历史' },
  { name: '/status', description: '显示会话状�? },
  ...
  { name: '/mode', description: '切换交互模式', subcommands: {
    ...
    ask: { description: '仅回答模�? },
  }},
  { name: '/thinking', description: '切换思考显�? },
  { name: '/compact', description: '切换紧凑模式' },
  { name: '/init', description: '初始化项�?NOVA.md' },
  ...
  { name: '/mcp', description: 'MCP 服务器管�?, subcommands: {
    status: { description: '查看状�? },
    list: { description: '列出服务�? },
    tools: { description: '列出工具', args: ['<server>'] },
  }},
  { name: '/skills', description: '技能管�?, subcommands: {
    list: { description: '列出技�? },
    use: { description: '激活技�?, args: ['<name>'] },
    info: { description: '查看技能详�?, args: ['<name>'] },
  }},
...
export class EnhancedCompleter {
    ...
    try {
      ...
      for (const [providerName, provider] of Object.entries(config.models.providers)) {
        for (const modelName of Object.keys(provider.models)) {
          // 同时支持 provider/model 和单�?model 两种格式
  ...
  getCompletions(input: string, cursorPosition?: number): CompletionCandidate[] {
    ...
    // /command 命令补全 - 使用原始 input 以保留尾随空�?
    if (trimmed.startsWith('/')) {
    ...
    for (const server of this.mcpServers) {
      if (server.name.toLowerCase().startsWith(lowerPartial)) {
        candidates.push({
          ...
          description: `MCP 服务�?(${server.status})`,
          type: 'mcp',
          priority: 80,
        });
      }
