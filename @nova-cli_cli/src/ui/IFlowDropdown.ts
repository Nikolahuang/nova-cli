...
export class IFlowDropdown {
  ...
  private async render(input: string): Promise<void> {
    ...
    console.log(chalk.blue(`â”?Use â†‘â†“ to navigate, Enter to select, Esc to cancel${' '.repeat(width - 64)}â”‚`));
    console.log(chalk.blue(`â”?Input: ${chalk.cyan(input)}${' '.repeat(width - 18 - input.length)}â”‚`));
    console.log(chalk.blue(`â”?{'â”€'.repeat(width)}â”¤`));
    ...
    for (let i = 0; i < displayCount; i++) {
      ...
      // Selection indicator
      const prefix = isSelected ? chalk.green('â–?') : chalk.gray('  ');
      const indent = isSelected ? ' ' : 'Â·';
      ...
      console.log(chalk.blue(`â”?{line.padEnd(width)}â”‚`));
    }
      ...
      const handleKeyPress = (key: string) => {
        switch (key) {
          ...
          case '\n':
            ...
            if (selected) {
              console.log(chalk.green(`âœ?Executing: ${selected.label}`));
              setTimeout(() => {
                if (selected.action) {
                  selected.action();
                }
