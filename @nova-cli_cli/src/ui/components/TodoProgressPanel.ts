...
export class TodoProgressPanel {
  ...
  private render(): void {
    ...
    const headerLine = C.brandDim(BOX.v) + ' ' + C.brand('📋') + ' ' + C.primary(headerText) + progressBar + ' '.repeat(Math.max(0, width - headerText.length - 18)) + C.brandDim(BOX.v);
  ...
  private renderStatsLine(stats: ReturnType<typeof this.getStats>, width: number): string {
    ...
    const statsText = parts.join(C.dim(' · '));
  ...
  private renderMiniProgressBar(pct: number, width: number): string {
    ...
    return C.muted('[') + C.success('█'.repeat(filled)) + C.dim('░'.repeat(empty)) + C.muted(']') + C.muted(` ${pct}%`);
  }
