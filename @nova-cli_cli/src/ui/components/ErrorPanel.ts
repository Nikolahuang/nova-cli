  ...
  private displayFull(errorInfo: ErrorInfo, context?: any): void {
    ...
    if (errorInfo.suggestion.length > 0 && this.options.showSuggestions) {
      ...
      switch (errorInfo.category) {
        case 'auth':
          console.log(chalk.cyan('  â€?nova auth set <provider>'));
          console.log(chalk.cyan('  â€?Check environment variables'));
          break;
        case 'config':
          console.log(chalk.cyan('  â€?nova config edit'));
          console.log(chalk.cyan('  â€?Check ~/.nova/config.yaml'));
          break;
        case 'network':
          console.log(chalk.cyan('  â€?Check internet connection'));
          console.log(chalk.cyan('  â€?Try nova ollama status'));
          break;
        case 'model':
          console.log(chalk.cyan('  â€?nova model list'));
          console.log(chalk.cyan('  â€?Check provider configuration'));
          break;
      }
    }
